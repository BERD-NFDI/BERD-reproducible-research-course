...
- Optional task for week 4: Try out the Makefile example from the course repository.  
  If you haven’t already, clone the [BERD course repository](https://github.com/BERD-NFDI/BERD-reproducible-research-course), then navigate into the `make-example` folder.  
  Run the following in your terminal:  
  `make data_clean/PlantGrowth_new.csv`  
  Then run the same command again and observe the output.  
  Next, open `preprocess.R`, add a harmless comment (e.g. `# This is just a test comment to trigger Make`), save the file, and run the command again.  
